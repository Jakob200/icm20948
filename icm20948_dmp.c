...
icm20948_status_e icm20948_init_dmp_sensor_with_defaults(icm20948_device_t *pdev)
{
    ...
    icm20948_status_e result = ICM_20948_STAT_OK; // Use result and worst_result to show if the
    ...
    result = icm20948_i2c_controller_configure_peripheral(
        pdev, 0, MAG_AK09916_I2C_ADDR, AK09916_REG_RSV2, 10, true, true, false, true, true, 0);
    ...
    // command each sample
    result = icm20948_i2c_controller_configure_peripheral(pdev, 1, MAG_AK09916_I2C_ADDR,
                                                          AK09916_REG_CNTL2, 1, false, true, false,
    ...
    if (result > worst_result)
        worst_result = result;   // Select Bank 3
    uint8_t mstODRconfig = 0x04; // Set the ODR configuration to 1100/2^4 = 68.75Hz
    result = icm20948_execute_w(pdev, AGB3_REG_I2C_MST_ODR_CONFIG, &mstODRconfig, 1);
    if (result > worst_result)
        worst_result = result; // Write one byte to the I2C_MST_ODR_CONFIG register

    // Configure clock source through PWR_MGMT_1
    // CLOCK_AUTO selects the best available clock source – PLL if ready, else use the Internal
    // oscillator
    ...
    if (result > worst_result)
        worst_result = result; // Select Bank 0
    uint8_t pwrMgmt2 = 0x40;   // Set the reserved bit 6 (pressure sensor disable?)
    result = icm20948_execute_w(pdev, AGB0_REG_PWR_MGMT_2, &pwrMgmt2, 1);
    if (result > worst_result)
        worst_result = result; // Write one byte to the PWR_MGMT_2 register
    ...
    icm20948_fss_t myFSS; // This uses a "Full Scale Settings" structure that can contain values for
                          // all configurable sensors
    ...
    myFSS.g = DPS_2000;   // (icm20948_gyro_config_1_fs_sel_e)
                          ...
                          // DPS_2000
    result =
        icm20948_set_full_scale(pdev, (ICM_20948_INTERNAL_ACC | ICM_20948_INTERNAL_GYR), myFSS);
    if (result > worst_result)
        worst_result = result;
    ...
    // result = intEnableOverflowFIFO( 0x1F ); if (result > worst_result) worst_result = result; //
    ...
    // mySmplrt.a = 8; // 112Hz
    result =
        icm20948_set_sample_rate(pdev, (ICM_20948_INTERNAL_ACC | ICM_20948_INTERNAL_GYR), mySmplrt);
    if (result > worst_result)
        worst_result = result;
    ...
    if (result > worst_result)
        worst_result = result; // Defaults to DMP_START_ADDRESS
    ...
    if (result > worst_result)
        worst_result = result; // Write accScale to ACC_SCALE DMP register
    // In order to output hardware unit data as configured FSR write 0x00040000 when FSR is 4g
    const unsigned char accScale2[4] = {0x00, 0x04, 0x00, 0x00};
    result = inv_icm20948_write_mems(pdev, ACC_SCALE2, 4, &accScale2[0]);
    if (result > worst_result)
        worst_result = result; // Write accScale2 to ACC_SCALE2 DMP register
    ...
    result =
        inv_icm20948_set_gyro_sf(pdev, 19, 3); // 19 = 55Hz (see above), 3 = 2000dps (see above)
    if (result > worst_result)
        worst_result = result;
...
static void pack_q30_be(int32_t v, unsigned char *b)
{
...
{
    static const char *TAG = "icm20948_seed";

    // Let accel/mag settle after DMP init configures the I2C slave
    ...
    for (int i = 0; i < 10; i++) {
        ...
        if (icm20948_get_agmt(pdev, &agmt) == ICM_20948_STAT_OK) {
            ax += (float)agmt.acc.axes.x;
            ay += (float)agmt.acc.axes.y;
            az += (float)agmt.acc.axes.z;
            mx += (float)agmt.mag.axes.x;
            my += (float)agmt.mag.axes.y;
            mz += (float)agmt.mag.axes.z;
            valid++;
        }
    ...
    if (valid == 0) {
        ESP_LOGW(TAG, "No valid raw samples; keeping identity quaternion");
    ...
    if (anorm < 1.0f) {
        ESP_LOGW(TAG, "Accel norm too small; keeping identity quaternion");
        return ICM_20948_STAT_OK;
    }
    ax /= anorm;
    ay /= anorm;
    ...
    if (mnorm >= 100.0f) {
        ...
        ESP_LOGW(TAG, "Mag norm %.1f < 100 LSB; seeding pitch/roll only", (double)mnorm);
    }

    ESP_LOGI(TAG, "Seed: yaw=%.1f pitch=%.1f roll=%.1f deg", (double)(yaw * 180.0f / (float)M_PI),
             (double)(pitch * 180.0f / (float)M_PI), (double)(roll * 180.0f / (float)M_PI));
    ...
    // Q0_QUAT6..Q3_QUAT6 seed the DMP 6-axis orientation filter (Q30 big-endian)
    unsigned char buf[4];
