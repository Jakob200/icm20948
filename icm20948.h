...
// There are two versions of the InvenSense DMP firmware for the ICM20948 - with slightly different
// sizes
...
typedef enum {
    ...
    ICM_20948_STAT_INVALID_SENSOR, // Tried to apply a function to a sensor that does not support it
                                   // (e.g. DLPF to the temperature sensor)
...
typedef struct {
    uint8_t a;
    uint8_t g;
} icm20948_dlpcfg_t; // Holds digital low pass filter settings. Members are type
...
typedef struct {
    ...
    uint8_t
        _last_mems_bank; // Keep track of which bank was selected last - to avoid unnecessary writes
    ...
    uint32_t
        _enabled_Android_intr_0; // Keep track of which Android sensor interrupts are enabled: 0-31
    uint32_t
        _enabled_Android_intr_1; // Keep track of which Android sensor interrupts are enabled: 32-
...
// icm20948_status_e ICM_20948_Startup( icm20948_device_t* pdev ); // For the time being this
// performs a standardized startup routine

icm20948_status_e
icm20948_link_serif(icm20948_device_t *pdev,
                    const icm20948_serif_t *s); // Links a SERIF structure to the device

// use the device's serif to perform a read or write
icm20948_status_e icm20948_execute_r(
    icm20948_device_t *pdev, uint8_t regaddr, uint8_t *pdata,
    uint32_t len); // Executes a R or W witht he serif vt as long as the pointers are not null
icm20948_status_e icm20948_execute_w(icm20948_device_t *pdev, uint8_t regaddr, uint8_t *pdata,
                                     uint32_t len);

// Single-shot I2C on Master IF
icm20948_status_e icm20948_i2c_controller_periph4_txn(icm20948_device_t *pdev, uint8_t addr,
                                                      uint8_t reg, uint8_t *data, uint8_t len,
                                                      bool Rw, bool send_reg_addr);
icm20948_status_e icm20948_i2c_master_single_w(icm20948_device_t *pdev, uint8_t addr, uint8_t reg,
                                               uint8_t *data);
icm20948_status_e icm20948_i2c_master_single_r(icm20948_device_t *pdev, uint8_t addr, uint8_t reg,
                                               uint8_t *data);
...
icm20948_status_e icm20948_low_power(icm20948_device_t *pdev,
                                     bool on); // Set low power mode for the chip
icm20948_status_e
icm20948_set_clock_source(icm20948_device_t *pdev,
                          icm20948_pwr_mgmt_1_clksel_e source); // Choose clock source
icm20948_status_e icm20948_get_who_am_i(icm20948_device_t *pdev,
                                        uint8_t *whoami); // Return whoami in out prarmeter
icm20948_status_e icm20948_check_id(
    icm20948_device_t *pdev); // Return 'ICM_20948_STAT_OK' if whoami matches ICM_20948_WHOAMI
icm20948_status_e icm20948_data_ready(icm20948_device_t *pdev); // Returns 'Ok' if data is ready

// Interrupt Configuration
icm20948_status_e
icm20948_int_pin_cfg(icm20948_device_t *pdev, icm20948_int_pin_cfg_t *write,
                     icm20948_int_pin_cfg_t *read); // Set the INT pin configuration
icm20948_status_e icm20948_int_enable(
    icm20948_device_t *pdev, icm20948_int_enable_t *write,
    icm20948_int_enable_t
        *read); // Write and or read the interrupt enable information. If non-null the write
                // operation occurs before the read, so as to verify that the write was successful

// WoM Enable Logic configuration
icm20948_status_e
icm20948_wom_logic(icm20948_device_t *pdev, icm20948_accel_intel_ctrl_t *write,
                   icm20948_accel_intel_ctrl_t *read); // Enable or disable WoM Logic

// WoM Threshold Level Configuration
icm20948_status_e icm20948_wom_threshold(
    icm20948_device_t *pdev, icm20948_accel_wom_thr_t *write,
    icm20948_accel_wom_thr_t
        *read); // Write and or read the Wake on Motion threshold. If non-null the write operation
                // occurs before the read, so as to verify that the write was successful

// Internal Sensor Options
icm20948_status_e
icm20948_set_sample_mode(icm20948_device_t *pdev, icm20948_internal_sensor_id_bm sensors,
                         icm20948_lp_config_cycle_e mode); // Use to set accel, gyro, and I2C master
                                                           // into cycled or continuous modes
icm20948_status_e icm20948_set_full_scale(icm20948_device_t *pdev,
                                          icm20948_internal_sensor_id_bm sensors,
                                          icm20948_fss_t fss);
icm20948_status_e icm20948_set_dlpf_cfg(icm20948_device_t *pdev,
                                        icm20948_internal_sensor_id_bm sensors,
                                        icm20948_dlpcfg_t cfg);
icm20948_status_e icm20948_enable_dlpf(icm20948_device_t *pdev,
                                       icm20948_internal_sensor_id_bm sensors, bool enable);
icm20948_status_e icm20948_set_sample_rate(icm20948_device_t *pdev,
                                           icm20948_internal_sensor_id_bm sensors,
...
icm20948_status_e icm20948_i2c_controller_configure_peripheral(
    icm20948_device_t *pdev, uint8_t peripheral, uint8_t addr, uint8_t reg, uint8_t len, bool Rw,
 ...
 */
icm20948_status_e inv_icm20948_firmware_load(icm20948_device_t *pdev, const unsigned char *data,
                                             unsigned short size, unsigned short load_addr);
/**
 ...
 */
icm20948_status_e inv_icm20948_write_mems(icm20948_device_t *pdev, unsigned short reg,
 ...
 *  @return     0 if successful.
 */
icm20948_status_e inv_icm20948_read_mems(icm20948_device_t *pdev, unsigned short reg,
                                         unsigned int length, unsigned char *data);

icm20948_status_e inv_icm20948_set_dmp_sensor_period(icm20948_device_t *pdev,
                                                     enum DMP_ODR_Registers odr_reg,
                                                     uint16_t interval);
icm20948_status_e inv_icm20948_enable_dmp_sensor(icm20948_device_t *pdev,
                                                 enum inv_icm20948_sensor sensor,
                                                 int state); // State is actually boolean
icm20948_status_e inv_icm20948_enable_dmp_sensor_int(icm20948_device_t *pdev,
                                                     enum inv_icm20948_sensor sensor,
                                                     int state); // State is actually boolean
...
icm20948_status_e inv_icm20948_set_gyro_sf(icm20948_device_t *pdev, unsigned char div,
                                           int gyro_level);
...
SERIAL_PORT.print(F("Enabling the I2C master returned "));
SERIAL_PORT.println(myICM.statusString()); myICM.i2cControllerConfigurePeripheral ( 0,
MAG_AK09916_I2C_ADDR, REG_ST1, 9, true, true, false, false, false );
SERIAL_PORT.print(F("Configuring the magnetometer peripheral returned "));
SERIAL_PORT.println(myICM.statusString());

// Operate the I2C master in duty-cycled mode
myICM.setSampleMode( (ICM_20948_INTERNAL_MST | ICM_20948_INTERNAL_GYR), SAMPLE_MODE_CYCLED ); //
options: SAMPLE_MODE_CONTINUOUS or SAMPLE_MODE_CYCLED
*/
